...
Download the Windows installer `BlueNotebook-Setup-5.0.1.exe` abs it's hash file `BlueNotebook-Setup-5.0.1.exe.hash`

To verify the hash 

```powershell
$expected = (Get-Content "BlueNotebook-Setup-5.0.1.exe.hash" | Select-String "Hash").ToString().Split(":")[1].Trim()
$actual = (Get-FileHash -Path "BlueNotebook-Setup-5.0.1.exe" -Algorithm SHA256).Hash
if ($expected -eq $actual) { Write-Host "OK - Hash " -ForegroundColor Green } else { Write-Host "ERROR -  different Hash" -ForegroundColor Red }
```
Install BlueNotebook using the Windows installer `BlueNotebook-Setup-5.0.1.exe`
...
# Make it Runnable
chmod +x BlueNotebook-5.0.1-x86_64.AppImage
# Run
./BlueNotebook-5.0.1-x86_64.AppImage
```
Manual update from the previous version using zsync or AppImageUpdate with BlueNotebook-5.0.1-x86_64.AppImage.zsync
...
$ ls -al *.flatpak
BlueNotebook-5.0.1.flatpak

# Install the local bundle
$ flatpak install --bundle --user BlueNotebook-5.0.1.flatpak
...
docker pull jmdigne/bluenotebook:5.0.1
...
$ docker pull ghcr.io/lephotographelibre/bluenotebook:5.0.1
...
docker run -it --rm \
    ...
    -v ~/bluenotebook_docker/BlueNotebookBackup:/home/appuser/BlueNotebookBackup \
    --user=$(id -u):$(id -g) \
    ghcr.io/lephotographelibre/bluenotebook:5.0.1
```
