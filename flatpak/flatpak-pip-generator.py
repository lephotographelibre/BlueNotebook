...
with tempfile.TemporaryDirectory(prefix=tempdir_prefix) as tempdir:
    ...
    for filename in os.listdir(tempdir):
        if not filename.endswith(("bz2", "any.whl", "gz", "xz", "zip")):
            ...
            name = get_package_name(filename)
            if name.lower() in ["pymupdf", "numpy", "pandas"]:
                print(f"Skipping source download for {name}, using binary wheel")
                continue
            try:
                ...
                print(
                    f"WARNING: Source not found for {name} {version}. Using binary wheel {filename}"
                )
