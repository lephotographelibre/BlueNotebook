...
*   **Filtres Syntaxiques** :
    *   Les mots de moins de 3 caractères sont ignorés. <!-- Word indexing and search removed in V3.1.1 -->
    *   Les chaînes ne contenant aucune lettre (ex: nombres seuls comme `2024`, symboles comme `---`) sont exclues. <!-- Word indexing and search removed in V3.1.1 -->
    *   Les tags (`@@...`) sont spécifiquement ignorés pour ne pas polluer l'index de mots. <!-- Word indexing and search removed in V3.1.1 -->
...
3.  **Collecte des Données** : Pour la requête donnée, l'application récupère toutes les "details" (occurrences) correspondantes dans le fichier JSON. Chaque détail contient la date, le contexte et le nom du fichier. <!-- Word indexing and search removed in V3.1.1 -->
...
sequenceDiagram
    participant User
    participant TagCloudPanel
    participant NavigationPanel
    participant MainWindow

    User->>TagCloudPanel: Clique sur "@@projet"
    TagCloudPanel->>NavigationPanel: émet tag_clicked("@@projet")
    ...
    MainWindow->>MainWindow: perform_search("@@projet") lit index_tags.json
