# BlueNotebook - Documentation Technique V1.9.1
...
Pour des raisons de sécurité, `QWebEngine` bloque par défaut l'accès aux fichiers locaux. Pour contourner ce problème, la méthode `setHtml` est appelée avec une URL de base `file:///`, autorisant ainsi le chargement des images via des chemins absolus.

### Personnalisation de l'Interface

BlueNotebook offre des options de personnalisation avancées pour l'éditeur et l'aperçu HTML, principalement gérées via la boîte de dialogue `PreferencesDialog`.

#### Thèmes de l'Éditeur Markdown

*   **Principe** : L'utilisateur peut sauvegarder l'ensemble des paramètres d'affichage de l'éditeur (polices, couleurs de fond, de texte, de syntaxe, etc.) sous la forme d'un "thème".
*   **Stockage** : Les thèmes sont sauvegardés sous forme de fichiers JSON dans le répertoire `resources/themes/`. Chaque fichier contient les informations sur les polices et les couleurs.
*   **Gestion via les Préférences** :
    *   **Sauvegarder un thème** : Dans `Préférences > Affichage > Editeur Markdown`, le bouton `💾 Sauvegarder comme thème` ouvre une boîte de dialogue pour nommer le nouveau thème. Les paramètres d'affichage actuels sont alors écrits dans un nouveau fichier JSON.
    *   **Charger un thème** : Le bouton `🎨 Sélectionner un thème` liste les thèmes disponibles depuis le répertoire `resources/themes/`. La sélection d'un thème charge ses valeurs dans l'interface des préférences, permettant à l'utilisateur de les prévisualiser et de les appliquer.
*   **Implémentation** : La logique est gérée dans `preferences_dialog.py` par les méthodes `_save_as_theme` et `_load_theme`.

#### Thèmes CSS pour l'Aperçu HTML

*   **Principe** : L'apparence de l'aperçu HTML est entièrement contrôlée par des feuilles de style CSS, permettant une personnalisation complète du rendu.
*   **Stockage** : Les feuilles de style sont stockées dans le répertoire `resources/css_preview/`.
*   **Gestion via les Préférences** :
    *   **Sélection** : Dans `Préférences > Affichage > Aperçu HTML`, le bouton `🎨 Sélectionner un thème CSS` permet à l'utilisateur de choisir l'une des feuilles de style disponibles dans le répertoire.
    *   **Persistance** : Le nom du fichier CSS sélectionné est sauvegardé dans `settings.json` sous la clé `preview.css_theme`.
*   **Implémentation** :
    *   `preferences_dialog.py` gère la sélection du fichier.
    *   `preview.py`, via sa méthode `set_css_theme`, charge le contenu du fichier CSS choisi et l'applique au template HTML généré. Au démarrage, `main_window.py` appelle cette méthode pour appliquer le thème stocké dans les préférences.
...
sequenceDiagram
    ...
    User->>MainWindow: Ouvre les Préférences
    MainWindow->>PreferencesDialog: Crée et affiche la dialogue
    User->>PreferencesDialog: Va dans "Aperçu HTML" et clique "Sélectionner thème"
    PreferencesDialog->>PreferencesDialog: Affiche la liste des fichiers de `resources/css_preview/`
    User->>PreferencesDialog: Sélectionne "github-markdown-dark_preview.css"
    PreferencesDialog->>PreferencesDialog: Mémorise "github-markdown-dark_preview.css"
    User->>PreferencesDialog: Clique sur "Valider"
    PreferencesDialog-->>MainWindow: Dialogue acceptée
    MainWindow->>MainWindow: settings_manager.set("preview.css_theme", "...")
    MainWindow->>MainWindow: settings_manager.save_settings()
    MainWindow->>MainWindow: apply_settings()
    MainWindow->>PreviewPanel: set_css_theme("github-markdown-dark_preview.css")
    PreviewPanel->>PreviewPanel: Charge le CSS et met à jour l'aperçu
```
...
Cet onglet est dédié à la personnalisation visuelle de la zone d'écriture. Il permet de configurer les couleurs de fond, de police, et de nombreux éléments de la coloration syntaxique (titres, gras, italique, code, liens, commentaires HTML, etc.).
...
-   Le contenu de l'aide est un fichier HTML (`aide_en_ligne.html`) situé dans `resources/html/`.
...
-   **Insertion** : L'utilisateur peut insérer cette citation dans l'éditeur via le menu `Insérer -> Citation du jour`. Elle sera formatée en tant que citation Markdown (`>`).
...
-   **`run_bluenotebook.sh` (Linux/macOS)** :
    ...
    -   Détecte l'environnement de bureau (KDE, GNOME, etc.) pour définir la variable `QT_QPA_PLATFORMTHEME`, assurant une meilleure intégration visuelle.
...
*   **Champ de Recherche** : Situé dans le panneau de `Navigation`, un champ de saisie permet à l'utilisateur de taper un mot ou un tag.
    ...
    *   Un menu déroulant (bouton `▼`) permet de sélectionner un tag parmi tous ceux indexés, ce qui l'insère correctement dans le champ. <!-- Word indexing and search removed in V3.1.1 -->
...
*Documentation technique v1.9.1 - BlueNotebook*  
*Générée le : 28 Septembre 2025*