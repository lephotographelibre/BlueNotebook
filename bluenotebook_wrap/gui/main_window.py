...
class GpxSourceDialog(QDialog):
    ...
    def _browse_file(self):
        path, _ = QFileDialog.getOpenFileName(
            self, self.tr("Sélectionner un fichier GPX"), self.tr(""), self.tr("Fichiers GPX (*.gpx)")
        )
        if path:
...
class PdfSourceDialog(QDialog):
    ...
    def _browse_file(self):
        path, _ = QFileDialog.getOpenFileName(
            self, self.tr("Sélectionner un fichier PDF"), self.tr(""), self.tr("Fichiers PDF (*.pdf)")
        )
        if path:
            self.path_edit.setText(path)
...
class InsertLinkDialog(QDialog):
    ...
    def _browse_local_file(self):
        ...
        path, _ = QFileDialog.getOpenFileName(
            self, self.tr("Sélectionner un fichier local"), start_dir, self.tr("Tous les fichiers (*)")
        )
        if path:
...
class MainWindow(QMainWindow):
    ...
    def set_application_icon(self):
        ...
        print(self.tr("ℹ️ Aucune icône trouvée, utilisation de l'icône par défaut"))
    ...
    def _create_actions(self):
        ...
        self.redo_action = QAction(
            self.tr("Rétablir"), self, shortcut=QKeySequence.Redo, triggered=self.editor.redo
        )
        ...
        self.insert_template_action = QAction(
            ...
            self,
            statusTip=self.tr("Insérer le contenu d'un modèle à la position du curseur"),
        ...
        self.insert_amazon_book_action = QAction(
            self.tr("Amazon ISBN"),
            self,
            statusTip=self.tr("Insérer les informations d'un livre depuis Amazon via son ISBN"),
    ...
    def _setup_format_menu(self, format_menu):
        ...
        style_actions_data = [
            ...
            (self.tr("Barré (~~texte~~)"), self.tr("strikethrough")),
            (self.tr("Surligné (==texte==)"), self.tr("highlight")),
        ]
    ...
    def _setup_insert_menu(self, insert_menu):
        ...
        insert_actions_data = [
            ...
            (self.tr("🔗 Lien"), self.tr("markdown_link")),
            (self.tr("Lien URL/Email"), self.tr("url_link")),
            (self.tr("📎 Attachement"), self.tr("attachment")),
        ]
        ...
        emoji_actions_data = [
            (self.tr("📖 Livre"), self.tr("📖")),
            (self.tr("🎵 Musique"), self.tr("🎵")),
            (self.tr("📚 À Lire"), self.tr("📚")),
            (self.tr("🎬 À Regarder"), self.tr("🎬")),
            (self.tr("🎧 A Ecouter"), self.tr("🎧")),
            (self.tr("✈️ Voyage"), self.tr("✈️")),
            (self.tr("❤️ Santé"), self.tr("❤️")),
            (self.tr("☀️ Soleil"), self.tr("☀️")),
            (self.tr("☁️ Nuage"), self.tr("☁️")),
            (self.tr("🌧️ Pluie"), self.tr("🌧️")),
            (self.tr("🌬️ Vent"), self.tr("🌬️")),
            (self.tr("😊 Content"), self.tr("😊")),
            (self.tr("😠 Mécontent"), self.tr("😠")),
            (self.tr("😢 Triste"), self.tr("😢")),
            (self.tr("✅ Fait"), self.tr("✅")),
            (self.tr("❌ Annulé"), self.tr("❌")),
            (self.tr("⚠️ Attention"), self.tr("⚠️")),
            (self.tr("📝 Mémo"), self.tr("📝")),
            (self.tr("❓ Question"), self.tr("❓")),
            (self.tr("❗ Exclamation"), self.tr("❗")),
        ]
    ...
    def setup_journal_directory(self):
        ...
        if self.journal_directory:
            ...
            print(self.tr("⚠️ Répertoire du journal non défini."))
    ...
    def save_as_template(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, self.tr("Erreur"), self.tr("Impossible de sauvegarder le modèle :\n%1").arg(str(e))
            )
    ...
    def check_save_changes(self):
        ...
        if self.is_modified:
            ...
            msg_box.setText(
                self.tr("Le fichier a été modifié. Voulez-vous sauvegarder les modifications ?")
            )
    ...
    def show_quote_of_the_day(self):
        ...
        if self.settings_manager.get(self.tr("integrations.show_quote_of_the_day")):
            ...
            if self.daily_quote and self.daily_author:
                ...
                msg_box.setText(
                    self.tr("<blockquote><i>« %1 »</i></blockquote>").arg(self.daily_quote)
                )
    ...
    def insert_quote_of_the_day(self):
        ...
        if self.daily_quote and self.daily_author:
            ...
            QMessageBox.warning(
                self, self.tr("Erreur"), self.tr("Impossible de récupérer la citation du jour.")
            )
    ...
    def insert_youtube_video(self):
        ...
        if selected_text:
            ...
            dialog.setLabelText(self.tr("Entrez l'URL de la vidéo ou playlist Youtube:"))
    ...
    def insert_gps_map(self):
        ...
        if not self.journal_directory:
            QMessageBox.warning(
                ...
                self.tr("Veuillez définir un répertoire de journal avant d'insérer une carte."),
            )
            return
    ...
    def insert_gpx_trace(self):
        ...
        if not self.journal_directory:
            QMessageBox.warning(
                self,
                self.tr("Journal non défini"),
                self.tr("Veuillez définir un répertoire de journal avant d'insérer une trace GPX."),
            )
            return
        ...
        if markdown_block:
            ...
            QMessageBox.critical(self, self.tr("Erreur de création de la trace"), message)
    ...
    def insert_html_image(self):
        ...
        if exif_caption:
            reply = QMessageBox.question(
                ...
                self.tr("Des données EXIF ont été trouvées dans l'image. ")
                self.tr("Voulez-vous les insérer sous l'image ?"),
    ...
    def _handle_markdown_link(self):
        ...
        # Cas 2: Le fichier est en dehors du journal
            ...
            reply = QMessageBox.question(
                ...
                self.tr("Le fichier que vous avez sélectionné est en dehors du répertoire du journal.\n\n")
                self.tr("Voulez-vous le copier dans le journal pour garantir la portabilité de vos notes ?"),
                QMessageBox.Yes | QMessageBox.No,
                QMessageBox.Yes,
            )
            ...
            try:
                ...
                self.statusbar.showMessage(
                    self.tr("Fichier copié dans %1").arg(final_relative_path), 4000
                )
            except Exception as e:
                QMessageBox.critical(
        ...
        if final_relative_path:
            ...
            markdown_link = self.tr("🔗 [[[%1]]](%2)").arg(link_text).arg(encoded_path)
            self.editor.insert_text(markdown_link)
    ...
    def insert_sun_moon_data(self):
        ...
        if not all([city, latitude, longitude]):
            QMessageBox.warning(
                ...
                self.tr("Veuillez configurer votre ville dans 'Préférences > Intégrations' ")
                self.tr("pour utiliser cette fonctionnalité."),
            )
            return

        # Afficher un message d'attente
        self.statusbar.showMessage(self.tr("Récupération des données astronomiques..."), 0)
    ...
    def on_book_search_finished(self, markdown_fragment, has_selection):
        ...
        self.statusbar.showMessage(self.tr("Informations du livre insérées avec succès."), 5000)
    ...
    def on_transcript_finished(self, transcript, lang):
        ...
        reply = QMessageBox.question(
            ...
            self.tr("Une transcription en '%1' existe pour cette vidéo. Voulez-vous l'ajouter ?").arg(lang),
            QMessageBox.Yes | QMessageBox.No,
            QMessageBox.Yes,
        )
        if reply == QMessageBox.Yes:
    ...
    def on_no_transcript(self):
        ...
        print(self.tr("ℹ️ Aucune transcription trouvée pour cette vidéo."))
    ...
    def on_pdf_convert_finished(self, markdown_content):
        ...
        self.expand_outline()
        QMessageBox.information(
            self, self.tr("Conversion terminée"), self.tr("Le fichier PDF a été converti avec succès.")
        )
    ...
    def _on_journal_backup_finished(self, backup_path: str):
        ...
        print(self.tr("🔁 Sauvegarde du journal terminée avec succès dans : %1").arg(backup_path))
    ...
    def refresh_tag_index(self, event):
        ...
        if event.button() == Qt.LeftButton:
            print(self.tr("🔄 Rafraîchissement manuel de l'index des tags demandé."))
    ...
    def refresh_tag_index_from_nav(self):
        ...
        print(
            self.tr("🔄 Rafraîchissement manuel de l'index des tags demandé depuis la navigation.")
        )
        self.tag_index_status_label.setText(self.tr("Indexation en cours..."))
        self.tag_index_status_label.repaint()
        self.start_initial_indexing()
    ...
    def update_indexing_status_label(self):
        ...
        print(self.tr("✅ %1").arg(full_message))
    ...
    def update_calendar_highlights(self):
        ...
        try:
            ...
            print(
                self.tr("Répertoire du journal non trouvé pour la mise à jour du calendrier: %1").arg(self.journal_directory)
            )
        self.update_tag_cloud()
    ...
    def open_preferences(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            QMessageBox.information(
                ...
                self.tr("Certains changements, comme la police de l'application, nécessitent un redémarrage pour être pleinement appliqués."),
            )
    ...
    def open_file_from_search(self, filename: str, line_number: int):
        ...
        if file_path.exists():
            ...
            QMessageBox.warning(
                self, self.tr("Fichier non trouvé"), self.tr("Le fichier '%1' n'existe plus.").arg(filename)
            )
