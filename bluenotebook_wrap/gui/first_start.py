...
class FirstStartWindow(QDialog):
    ...
    def _setup_ui(self):
        ...
        logo_path = (
            base_path / self.tr("resources") / self.tr("images") / self.tr("bluenotebook_256-x256_fond_blanc.png")
        )
        if logo_path.exists():
        ...
        self.journal_path_edit.setText(str(Path.home() / self.tr("BlueNotebookJournal")))
        ...
        self.backup_path_edit.setText(str(Path.home() / self.tr("BlueNotebookBackup")))
    ...
    def _select_journal_directory(self):
        ...
        if parent_dir:
            self.journal_path_edit.setText(
                str(Path(parent_dir) / self.tr("BlueNotebookJournal"))
            )
    ...
    def _select_backup_directory(self):
        ...
        if parent_dir:
            self.backup_path_edit.setText(str(Path(parent_dir) / self.tr("BlueNotebookBackup")))
    ...
    def _finish_setup(self):
        ...
        try:
            if journal_path.exists() and any(journal_path.iterdir()):
                reply = QMessageBox.question(
                    ...
                    self.tr("Voulez-vous quand même l'utiliser comme journal ?"),
            ...
            for sub_dir in [self.tr("notes"), self.tr("images"), self.tr("attachments"), self.tr("gpx")]:
                (journal_path / sub_dir).mkdir(exist_ok=True)

        except Exception as e:
            QMessageBox.critical(
                self, self.tr("Erreur"), self.tr("Impossible de créer le répertoire du journal :\n%1").arg(e)
            )
            return
        ...
        self.settings_manager.set(self.tr("app.language"), lang_map.get(selected_lang, self.tr("en_US")))
        self.settings_manager.set(self.tr("journal.directory"), str(journal_path))
        self.settings_manager.set(self.tr("backup.last_directory"), str(backup_path))
