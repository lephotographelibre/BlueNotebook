...
class PreferencesDialog(QDialog):
    ...
    def _create_general_tab(self):
        ...
        current_lang = self.settings_manager.get(self.tr("app.language"), self.tr("fr_FR"))
        if current_lang == self.tr("en_US"):
        ...
        app_font_family = self.settings_manager.get(self.tr("ui.app_font_family"), self.tr("Noto Sans"))
        app_font_size = self.settings_manager.get(self.tr("ui.app_font_size"), 10)
        ...
        show_indexing_stats = self.settings_manager.get(self.tr("ui.show_indexing_stats"), True)
        self.show_indexing_stats_checkbox.setChecked(
            str(show_indexing_stats).lower() == self.tr("true")
        )
        ...
        self.excluded_tags_edit.setPlainText(self.tr(", ").join(sorted(excluded_tags_list)))
    ...
    def _create_markdown_editor_sub_tab(self):
        ...
        # Ligne de séparation
        layout.addWidget(QLabel(self.tr("")), row, 0, 1, 4)
        row += 1
        ...
        show_line_numbers_setting = self.settings_manager.get(
            self.tr("editor.show_line_numbers"), False
        )
        is_checked = str(show_line_numbers_setting).lower() == self.tr("true")
        ...
        code_font_family = self.settings_manager.get(
            self.tr("editor.code_font_family"), self.tr("Consolas, Monaco, monospace")
        )
        ...
        colors_config = [
            ...
            (
                ...
                self.tr("editor.background_color"),
                self.tr("current_color"),
                self.tr("color_button"),
            ),
            (
                self.tr("Police éditeur:"),
                self.tr("editor.text_color"),
                self.tr("current_text_color"),
                self.tr("text_color_button"),
            ),
            (
                self.tr("Titres Markdown:"),
                self.tr("editor.heading_color"),
                self.tr("current_heading_color"),
                self.tr("heading_color_button"),
            ),
            (
                self.tr("Listes Markdown:"),
                self.tr("editor.list_color"),
                self.tr("current_list_color"),
                self.tr("list_color_button"),
            ),
            (
                self.tr("Texte sélectionné:"),
                self.tr("editor.selection_text_color"),
                self.tr("current_selection_text_color"),
                self.tr("selection_text_color_button"),
            ),
            (
                self.tr("Texte code inline:"),
                self.tr("editor.inline_code_text_color"),
                self.tr("current_inline_code_text_color"),
                self.tr("inline_code_text_color_button"),
            ),
            (
                self.tr("Fond code inline:"),
                self.tr("editor.inline_code_background_color"),
                self.tr("current_inline_code_bg_color"),
                self.tr("inline_code_bg_color_button"),
            ),
            (
                self.tr("Fond bloc code:"),
                self.tr("editor.code_block_background_color"),
                self.tr("current_code_block_bg_color"),
                self.tr("code_block_bg_color_button"),
            ),
            # Colonne 2
            (
                self.tr("Texte gras:"),
                self.tr("editor.bold_color"),
                self.tr("current_bold_color"),
                self.tr("bold_color_button"),
            ),
            (
                self.tr("Texte italique:"),
                self.tr("editor.italic_color"),
                self.tr("current_italic_color"),
                self.tr("italic_color_button"),
            ),
            (
                self.tr("Texte barré:"),
                self.tr("editor.strikethrough_color"),
                self.tr("current_strikethrough_color"),
                self.tr("strikethrough_color_button"),
            ),
            (
                self.tr("Fond surlignage:"),
                self.tr("editor.highlight_color"),
                self.tr("current_highlight_color"),
                self.tr("highlight_color_button"),
            ),
            (
                self.tr("Citations:"),
                self.tr("editor.quote_color"),
                self.tr("current_quote_color"),
                self.tr("quote_color_button"),
            ),
            (self.tr("Liens:"), self.tr("editor.link_color"), self.tr("current_link_color"), self.tr("link_color_button")),
            (
                self.tr("Commentaires HTML:"),
                "editor.html_comment_color",
                self.tr("current_html_comment_color"),
                self.tr("html_comment_color_button"),
            ),
            (
                self.tr("Tags (@@tag):"),
                self.tr("editor.tag_color"),
                self.tr("current_tag_color"),
                self.tr("tag_color_button"),
            ),
            (
                self.tr("Horodatage (HH:MM):"),
                self.tr("editor.timestamp_color"),
                self.tr("current_timestamp_color"),
                self.tr("timestamp_color_button"),
            ),
        ]
    ...
    def _create_html_preview_sub_tab(self):
        ...
        self.current_html_theme_label.setStyleSheet(self.tr("margin-left: 10px;"))
    ...
    def _select_css_theme(self, pdf=False):
        ...
        if pdf:
            css_dir = base_path / self.tr("resources") / self.tr("css_pdf")
            dialog_title = self.tr("Choisir un thème pour l'export PDF")
        else:
            css_dir = base_path / self.tr("resources") / self.tr("css_preview")
    ...
    def _create_pdf_export_sub_tab(self):
        ...
        self.current_pdf_theme_label.setStyleSheet(self.tr("margin-left: 10px;"))
    ...
    def showEvent(self, event):
        ...
        base_path = Path(__file__).parent.parent
        css_preview_dir = base_path / self.tr("resources") / self.tr("css_preview")
        css_pdf_dir = base_path / self.tr("resources") / self.tr("css_pdf")
    ...
    def _create_panels_tab(self):
        ...
        self.show_notes_checkbox.setChecked(str(show_notes).lower() == self.tr("true"))
        ...
        self.show_nav_checkbox.setChecked(str(show_nav).lower() == self.tr("true"))
        ...
        show_outline = self.settings_manager.get(self.tr("ui.show_outline_panel"), True)
        self.show_outline_checkbox.setChecked(str(show_outline).lower() == self.tr("true"))
        ...
        show_preview = self.settings_manager.get(self.tr("ui.show_preview_panel"), False)
        self.show_preview_checkbox.setChecked(str(show_preview).lower() == self.tr("true"))
        ...
        self.show_reader_checkbox.setChecked(str(show_reader).lower() == self.tr("true"))
    ...
    def _create_integrations_tab(self):
        ...
        show_quote = self.settings_manager.get(
            self.tr("integrations.show_quote_of_the_day"), False
        )
        self.show_quote_checkbox.setChecked(str(show_quote).lower() == self.tr("true"))
        ...
        self.youtube_integration_checkbox.setChecked(
            str(is_youtube_enabled).lower() == self.tr("true")
        )
        ...
        self.youtube_transcript_checkbox.setChecked(
            str(is_transcript_enabled).lower() == self.tr("true")
        )
        ...
        self.weather_city_edit.setText(
            self.settings_manager.get(self.tr("integrations.weather.city"))
        )
        ...
        self.weather_api_key_edit.setText(
            self.settings_manager.get(self.tr("integrations.weather.api_key"))
        )
        ...
        self.astro_city_edit.setText(
            self.settings_manager.get(self.tr("integrations.sun_moon.city"), self.tr(""))
        )
        ...
        self.astro_lat_edit.setText(
            self.settings_manager.get(self.tr("integrations.sun_moon.latitude"), self.tr(""))
        )
        ...
        self.astro_lon_edit.setText(
            self.settings_manager.get(self.tr("integrations.sun_moon.longitude"), self.tr(""))
        )
    ...
    def _search_city_coords(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, self.tr("Erreur de géolocalisation"), self.tr("Une erreur est survenue : %1").arg(e)
            )

    def _save_as_theme(self):
        """Sauvegarde les paramètres actuels comme un nouveau thème."""
        theme_name, ok = QInputDialog.getText(
            self, self.tr("Sauvegarder le thème"), self.tr("Nom du thème:"), text=self.tr("Mon Thème")
        )
        ...
        safe_name = self.tr("").join(
            c for c in theme_name if c.isalnum() or c in (self.tr(" "), self.tr("-"), self.tr("_"))
        ).strip()
        safe_name = safe_name.replace(self.tr(" "), self.tr("_"))
        ...
        base_path = Path(__file__).parent.parent
        themes_dir = base_path / self.tr("resources") / self.tr("themes")
        ...
        theme_data = {
            self.tr("name"): theme_name,
            self.tr("font"): {
                self.tr("family"): self.current_font.family(),
                self.tr("size"): self.current_font.pointSize(),
                self.tr("code_family"): self.current_code_font.family(),
            },
            self.tr("outline"): {
                self.tr("family"): self.current_outline_font.family(),
                self.tr("size"): self.current_outline_font.pointSize(),
            },
            self.tr("colors"): {
                self.tr("background_color"): self.current_color.name(),
                self.tr("text_color"): self.current_text_color.name(),
                self.tr("heading_color"): self.current_heading_color.name(),
                self.tr("list_color"): self.current_list_color.name(),
                self.tr("selection_text_color"): self.current_selection_text_color.name(),
                self.tr("inline_code_text_color"): self.current_inline_code_text_color.name(),
                self.tr("inline_code_background_color"): self.current_inline_code_bg_color.name(),
                self.tr("code_block_background_color"): self.current_code_block_bg_color.name(),
                self.tr("bold_color"): self.current_bold_color.name(),
                self.tr("italic_color"): self.current_italic_color.name(),
                self.tr("strikethrough_color"): self.current_strikethrough_color.name(),
                self.tr("highlight_color"): self.current_highlight_color.name(),
                self.tr("quote_color"): self.current_quote_color.name(),
                self.tr("link_color"): self.current_link_color.name(),
                self.tr("html_comment_color"): self.current_html_comment_color.name(),
                self.tr("tag_color"): self.current_tag_color.name(),
                self.tr("timestamp_color"): self.current_timestamp_color.name(),
            },
        }
        ...
        try:
            ...
            QMessageBox.critical(
                self, self.tr("Erreur"), self.tr("Impossible de sauvegarder le thème:\n%1").arg(e)
            )
    ...
    def _load_theme(self):
        ...
        for theme_file in theme_files:
            try:
                with open(theme_file, "r", encoding="utf-8") as f:
                    ...
                    name = theme_data.get(self.tr("name"), theme_file.stem)
        ...
        try:
            ...
            font_info = theme_data.get(self.tr("font"), {})
            self.current_font = QFont(
                font_info.get(self.tr("family"), self.tr("Arial")), font_info.get(self.tr("size"), 12)
            )
            self.font_button.setText(
                self.tr("%1, %2pt").arg(self.current_font.family()).arg(self.current_font.pointSize())
            )

            self.current_code_font = QFont(font_info.get(self.tr("code_family"), self.tr("Courier New")))
            self.code_font_button.setText(self.current_code_font.family())

            # Appliquer la police du plan
            outline_font_info = theme_data.get(
                self.tr("outline"), font_info
            )  # Fallback sur la police éditeur
            self.current_outline_font = QFont(
                outline_font_info.get(self.tr("family"), self.current_font.family()),
                outline_font_info.get(self.tr("size"), self.current_font.pointSize()),
            )
            ...
            colors = theme_data.get(self.tr("colors"), {})
            color_mappings = [
                (self.tr("background_color"), self.tr("current_color"), self.tr("color_button")),
                (self.tr("text_color"), self.tr("current_text_color"), self.tr("text_color_button")),
                (self.tr("heading_color"), self.tr("current_heading_color"), self.tr("heading_color_button")),
                (self.tr("list_color"), self.tr("current_list_color"), self.tr("list_color_button")),
                (
                    self.tr("selection_text_color"),
                    self.tr("current_selection_text_color"),
                    self.tr("selection_text_color_button"),
                ),
                (
                    self.tr("inline_code_text_color"),
                    self.tr("current_inline_code_text_color"),
                    self.tr("inline_code_text_color_button"),
                ),
                (
                    self.tr("inline_code_background_color"),
                    self.tr("current_inline_code_bg_color"),
                    self.tr("inline_code_bg_color_button"),
                ),
                (
                    self.tr("code_block_background_color"),
                    self.tr("current_code_block_bg_color"),
                    self.tr("code_block_bg_color_button"),
                ),
                (self.tr("bold_color"), self.tr("current_bold_color"), self.tr("bold_color_button")),
                (self.tr("italic_color"), self.tr("current_italic_color"), self.tr("italic_color_button")),
                (
                    self.tr("strikethrough_color"),
                    self.tr("current_strikethrough_color"),
                    self.tr("strikethrough_color_button"),
                ),
                (
                    self.tr("highlight_color"),
                    self.tr("current_highlight_color"),
                    self.tr("highlight_color_button"),
                ),
                (self.tr("quote_color"), self.tr("current_quote_color"), self.tr("quote_color_button")),
                (self.tr("link_color"), self.tr("current_link_color"), self.tr("link_color_button")),
                (
                    self.tr("html_comment_color"),
                    self.tr("current_html_comment_color"),
                    self.tr("html_comment_color_button"),
                ),
                (self.tr("tag_color"), self.tr("current_tag_color"), self.tr("tag_color_button")),
                (
                    self.tr("timestamp_color"),
                    self.tr("current_timestamp_color"),
                    self.tr("timestamp_color_button"),
                ),
            ]
            ...
            QMessageBox.critical(
                self, self.tr("Erreur"), self.tr("Impossible de charger le thème:\n%1").arg(e)
            )
    ...
    def _load_defaults_in_ui(self):
        ...
        font_info = defaults.get(self.tr("editor"), {})
        self.current_font = QFont(
            font_info.get(self.tr("font_family"), self.tr("Arial")), font_info.get(self.tr("font_size"), 12)
        )
        self.font_button.setText(
            self.tr("%1, %2pt").arg(self.current_font.family()).arg(self.current_font.pointSize())
        )

        self.current_code_font = QFont(font_info.get(self.tr("code_font_family"), self.tr("Courier New")))
        self.code_font_button.setText(self.current_code_font.family())

        # Appliquer la police du plan par défaut
        outline_font_info = defaults.get(self.tr("outline"), font_info)
        self.current_outline_font = QFont(
            outline_font_info.get(self.tr("font_family"), self.current_font.family()),
            outline_font_info.get(self.tr("font_size"), self.current_font.pointSize()),
        )
        self.outline_font_button.setText(
            self.tr("%1, %2pt").arg(self.current_outline_font.family()).arg(self.current_outline_font.pointSize())
        )
        # Appliquer les couleurs par défaut
        colors = defaults.get(self.tr("editor"), {})
        color_mappings = [
            (self.tr("background_color"), self.tr("current_color"), self.tr("color_button")),
            (self.tr("text_color"), self.tr("current_text_color"), self.tr("text_color_button")),
            (self.tr("heading_color"), self.tr("current_heading_color"), self.tr("heading_color_button")),
            ...
            (self.tr("highlight_color"), self.tr("current_highlight_color"), self.tr("highlight_color_button")),
            (self.tr("quote_color"), self.tr("current_quote_color"), self.tr("quote_color_button")),
            (self.tr("link_color"), self.tr("current_link_color"), self.tr("link_color_button")),
            (
                self.tr("html_comment_color"),
            ...
            (self.tr("tag_color"), self.tr("current_tag_color"), self.tr("tag_color_button")),
            (self.tr("timestamp_color"), self.tr("current_timestamp_color"), self.tr("timestamp_color_button")),
        ]
        ...
        default_html_theme = defaults.get(self.tr("preview"), {}).get(
    ...
    def accept(self):
        ...
        lang_map = {self.tr("Français"): self.tr("fr_FR"), self.tr("English"): self.tr("en_US")}
        selected_lang = self.lang_combo.currentText()
        self.settings_manager.set(self.tr("app.language"), lang_map.get(selected_lang, self.tr("fr_FR")))

        # ... (sauvegarde des autres paramètres)
        # V3.3.8 - Sauvegarde de la police de l'application
        self.settings_manager.set(self.tr("ui.app_font_family"), self.current_app_font.family())
        self.settings_manager.set(self.tr("ui.app_font_size"), self.current_app_font.pointSize())
        ...
        self.settings_manager.set(
            self.tr("integrations.weather.city"), self.weather_city_edit.text()
        )
        self.settings_manager.set(
            self.tr("integrations.weather.api_key"), self.weather_api_key_edit.text()
        )

        # Sauvegarde des paramètres d'intégration astro
        self.settings_manager.set(
            self.tr("integrations.sun_moon.city"), self.astro_city_edit.text()
        )
        self.settings_manager.set(
            self.tr("integrations.sun_moon.latitude"), self.astro_lat_edit.text()
        )
        self.settings_manager.set(
            self.tr("integrations.sun_moon.longitude"), self.astro_lon_edit.text()
        )
