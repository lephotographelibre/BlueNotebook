...
class QTextEditWithLineNumbers(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        # Couleurs par défaut
        self.line_number_bg_color = QColor(self.tr("#f0f0f0"))
        self.line_number_fg_color = QColor(self.tr("#808080"))
        self.current_line_number_color = QColor(self.tr("#000000"))
    ...
    def line_number_area_width(self):
        ...
        space = 8 + self.fontMetrics().horizontalAdvance(self.tr("9")) * digits + 8
        return space
    ...
    def highlight_current_line(self):
        ...
        if not self.isReadOnly():
            ...
            line_color = QColor(self.tr("#e8f4ff"))  # Bleu très clair
...
class MarkdownHighlighter(QSyntaxHighlighter):
    ...
    def __init__(self, document):
        ...
        self.heading_color = QColor(self.tr("#208bd7"))
        # Par défaut, la couleur des listes est la même que celle des titres
        self.list_color = QColor(self.tr("#208bd7"))
        # Couleurs par défaut pour le code inline
        self.inline_code_text_color = QColor(self.tr("#d6336c"))
        self.inline_code_background_color = QColor(self.tr("#f2f07f"))
        # Couleurs par défaut pour les styles de texte
        self.bold_color = QColor(self.tr("#448C27"))
        self.italic_color = QColor(self.tr("#448C27"))
        self.strikethrough_color = QColor(self.tr("#448C27"))
        self.highlight_color = QColor(self.tr("#FFC0CB"))
        # Couleurs par défaut pour les tags et horodatage
        self.tag_color = QColor(self.tr("#d73a49"))
        self.timestamp_color = QColor(self.tr("#005cc5"))
        # Couleur par défaut pour le fond des blocs de code
        self.code_block_background_color = QColor(self.tr("#f0f0f0"))
        # V1.7.2 Ajout Paramètre Affichages Couleurs
        # Couleurs par défaut pour les citations et les liens
        self.quote_color = QColor(self.tr("#2B303B"))
        self.link_color = QColor(self.tr("#0366d6"))
        # Couleur par défaut pour les commentaires HTML
        self.html_comment_color = QColor("#a4b5cf")

        # Police par défaut pour le code
        self.code_font_family = self.tr("Consolas, Monaco, monospace")
    ...
    def highlightBlock(self, text):
        ...
        bold_star_pattern = self.tr(r"\*\*(?!\s)(.*?)(?<!\s)\*\*")
        ...
        italic_star_pattern = self.tr(r"(?<!\*)\*(?!\s)(.*?)(?<!\s)\*(?!\*)")
        ...
        bold_underscore_pattern = self.tr(r"__(?!\s)(.*?)(?<!\s)__")
        ...
        italic_underscore_pattern = self.tr(r"(?<!\w)_([^_]+)_(?!\w)")
        ...
        # Barré (~~texte~~)
        strikethrough_pattern = self.tr(r"~~([^~]+)~~")
        ...
        list_pattern = self.tr(r"^\s*([-*+]|\d+\.|\-\s\[[ x]\])\s")
        ...
        html_comment_pattern = self.tr(r"<!--.*?-->")
...
class FindDialog(QDialog):
    ...
    def find_next(self):
        ...
        if hasattr(self.parent(), self.tr("find_text")):
            self.parent().find_text(self.search_edit.text())

    def replace_current(self):
        """Remplacer le texte actuel"""
        if hasattr(self.parent(), self.tr("replace_text")):
...
class LinkDialog(QDialog):
    ...
    def __init__(self, parent=None, selected_text=self.tr("")):
        super().__init__(parent)
        ...
        self.url_edit.setPlaceholderText(self.tr("https://example.com"))
    ...
    @staticmethod
    def get_link(parent=None, selected_text=self.tr("")):
        ...
        if selected_text.startswith("http://") or selected_text.startswith("https://"):
            ...
            dialog.text_edit.setText(self.tr(""))
...
class ImageSourceDialog(QDialog):
    ...
    def __init__(self, parent=None, initial_path=self.tr("")):
        super().__init__(parent)
...
class AttachmentSourceDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.path_edit = QLineEdit(self)
        self.path_edit.setPlaceholderText(
            self.tr("http://example.com/document.pdf ou /chemin/local/document.pdf")
        )
        path_layout.addWidget(self.path_edit)
    ...
    def _browse_file(self):
        ...
        file_filter = (
            ...
            self.tr("Documents (PDF, DOCX, TXT, ODT, KML) (*.pdf *.docx *.txt *.odt *.kml);;")
            self.tr("Images (*.png *.jpg *.jpeg *.gif *.bmp *.svg);;")
            "Archives (*.zip *.rar *.7z);;"
        ...
        path, _ = QFileDialog.getOpenFileName(
            self, self.tr("Sélectionner un fichier"), self.tr(""), file_filter
        )
        if path:
            self.path_edit.setText(path)
...
class MarkdownEditor(QWidget):
    ...
    def __init__(self, main_window=None):
        ...
        self.dynamic_styles = {
            self.tr("QTextEdit, QTextEditWithLineNumbers"): {},
        }
    ...
    def setup_ui(self):
        ...
        font = QFont(self.tr("'Droid Sans Mono', 'monospace', monospace"))
    ...
    def show_context_menu(self, position):
        ...
        if cursor.hasSelection():
            ...
            list_actions = [
                (self.tr("• Liste non ordonnée"), self.tr("ul")),
                (self.tr("1. Liste ordonnée"), self.tr("ol")),
                (self.tr("☑️ Liste de tâches"), self.tr("task_list")),
            ]
            ...
            bold_action.triggered.connect(lambda: self.format_text(self.tr("bold")))
            italic_action = style_menu.addAction(self.tr("Italique"))
            italic_action.triggered.connect(lambda: self.format_text(self.tr("italic")))
            strikethrough_action = style_menu.addAction(self.tr("Barré"))
            strikethrough_action.triggered.connect(
                lambda: self.format_text(self.tr("strikethrough"))
            )
            highlight_action = style_menu.addAction(self.tr("Surligné"))
            highlight_action.triggered.connect(lambda: self.format_text(self.tr("highlight")))
            ...
            inline_code_action.triggered.connect(
                lambda: self.format_text(self.tr("inline_code"))
            )
            code_block_action = code_menu.addAction(self.tr("Bloc de code"))
            code_block_action.triggered.connect(lambda: self.format_text(self.tr("code_block")))
            ...
            url_link_action.triggered.connect(lambda: self.format_text(self.tr("url_link")))
    ...
    def format_text(self, format_type):
        ...
        if not cursor.hasSelection():
            if format_type == self.tr("hr"):
                self.insert_text(self.tr("\n---\n"))
            elif format_type == self.tr("table"):
                table_template = self.tr("| En-tête 1 | En-tête 2 |\n|---|---|\n| Cellule 1 | Cellule 2 |\n| Cellule 3 | Cellule 4 |")
                self.insert_text(table_template)
            elif format_type == self.tr("time"):
                from datetime import datetime

                self.insert_text(self.tr("**%1**").arg(datetime.now().strftime('%H))
            elif format_type == self.tr("html_comment"):
                self.insert_text(self.tr("<!-- texte du commentaire -->"))
            return

        selected_text = cursor.selectedText()

        if format_type in [self.tr("h1"), self.tr("h2"), self.tr("h3"), self.tr("h4"), self.tr("h5")]:
            prefix = {
                self.tr("h1"): self.tr("# "),
                self.tr("h2"): self.tr("## "),
                self.tr("h3"): self.tr("### "),
        ...
        if format_type in [self.tr("quote"), self.tr("ul"), self.tr("ol"), self.tr("task_list")]:
            start_pos = cursor.selectionStart()
            end_pos = cursor.selectionEnd()
            cursor.setPosition(start_pos)
            cursor.movePosition(QTextCursor.StartOfLine)
        ...
        wrappers = {
            self.tr("bold"): self.tr("**"),
            self.tr("italic"): self.tr("*"),
            self.tr("strikethrough"): self.tr("~~"),
            self.tr("inline_code"): self.tr("`"),
            self.tr("highlight"): self.tr("=="),
        }
        ...
        elif format_type == self.tr("hr"):
            cursor.movePosition(QTextCursor.EndOfLine)
            cursor.insertText(self.tr("\n\n---\n"))

        elif format_type == self.tr("code_block"):
            if self.tr("\n") in selected_text:
                new_text = self.tr("```\n%1\n```").arg(selected_text)
            else:
                new_text = self.tr("```\n%1\n```").arg(selected_text)
        ...
        elif format_type == self.tr("url_link"):
    ...
    def _handle_markdown_link(self, selected_text=self.tr("")):
    ...
    def _copy_attachment_to_journal(self, source_path: str) -> Optional[str]:
        ...
        attachments_dir = journal_dir / self.tr("attachments")
        ...
        try:
            if is_remote:
                ...
                content_type = (
                    response.headers.get(self.tr("Content-Type"), self.tr("")).split(self.tr(";"))[0].strip()
                )
                allowed_mime_types = {  # Note: Garder cet ensemble synchronisé avec allowed_extensions
                    # Documents
                    self.tr("application/pdf"),
                    self.tr("application/msword"),
                    self.tr("application/vnd.openxmlformats-officedocument.wordprocessingml.document"),
                    self.tr("text/plain"),
                    self.tr("application/vnd.oasis.opendocument.text"),
                    self.tr("application/vnd.google-earth.kml+xml"),  # KML
                    # Images
                    self.tr("image/png"),
                    self.tr("image/jpeg"),
                    self.tr("image/gif"),
                    self.tr("image/bmp"),
                    self.tr("image/svg+xml"),
                    # Archives
                    self.tr("application/zip"),
                    self.tr("application/x-rar-compressed"),
                    self.tr("application/x-7z-compressed"),
                    # Audio/Vidéo
                    self.tr("audio/mpeg"),
                    self.tr("audio/wav"),
                    self.tr("video/mp4"),
                    self.tr("video/x-msvideo"),
                }

                if content_type not in allowed_mime_types:
                    allowed_types_str = self.tr("\n").join(sorted(list(allowed_mime_types)))
                    QMessageBox.warning(
                        self,
                ...
                if file_extension not in allowed_extensions:
                    allowed_ext_str = self.tr(", ").join(sorted(list(allowed_extensions)))
                    QMessageBox.warning(
                        self,
                        self.tr("Type de fichier non autorisé"),
            ...
            QMessageBox.critical(
                self, self.tr("Erreur de copie"), self.tr("Impossible de copier le fichier : %1").arg(e)
            )
            return None
    ...
    def _copy_image_to_journal(self, source_path: str, is_remote: bool = False) -> str:
        ...
        if is_remote:
            ...
            journal_images_dir = self.main_window.journal_directory / self.tr("images")
            journal_images_dir.mkdir(exist_ok=True)

            timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
            original_filename = Path(source_path).name.split(self.tr("?"))[
                0
    ...
    def clear_formatting(self):
        ...
        selected_text = cursor.selectedText()
        markdown_chars = self.tr(r"([#*_~`\[\]\(\)!<>])")

        cleaned_text = re.sub(
            self.tr(r"^\s*([#>\-]+\s*|\d+\.\s*)"), self.tr(""), selected_text, flags=re.MULTILINE
        )
        cleaned_text = re.sub(markdown_chars, "", cleaned_text)
        cleaned_text = re.sub(r"```.*?\n", "", cleaned_text)
        cleaned_text = cleaned_text.replace(self.tr("```"), self.tr(""))
    ...
    def _update_stylesheet(self):
        ...
        static_style = re.sub(
            self.tr(r"QTextEdit,\s*QTextEditWithLineNumbers\s*\{[^}]*\}"), self.tr(""), static_style
        )

        final_style = self.tr("\n").join(style_parts) + self.tr("\n") + static_style.strip()
        self.text_edit.setStyleSheet(final_style)

    def set_background_color(self, color_hex):
        """Définit la couleur de fond de l'éditeur."""
        self.dynamic_styles[self.tr("QTextEdit, QTextEditWithLineNumbers")][
    ...
    def set_text_color(self, color_hex):
        ...
        self.dynamic_styles[self.tr("QTextEdit, QTextEditWithLineNumbers")][self.tr("color")] = color_hex
        self._update_stylesheet()
