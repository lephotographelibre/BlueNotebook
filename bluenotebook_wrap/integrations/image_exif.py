...
"""
Module pour l'extraction et le formatage des données EXIF des images.
"""
...
def format_exif_as_markdown(image_path):
    ...
    if not any(
        [
            lat,
            exif_data.get(self.tr("DateTimeOriginal")),
            exif_data.get(self.tr("Model")),
        ]
    ):
        return None
    ...
    caption_text = self.tr(" : ").join(filter(None, caption_parts))
