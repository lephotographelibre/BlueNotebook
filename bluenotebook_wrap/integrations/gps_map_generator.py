...
def get_location_name(lat, lon):
    ...
    """
    try:
        geolocator = Nominatim(user_agent=self.tr("bluenotebook-app"))
        ...
        if location:
            ...
            city = (
                address.get(self.tr("city"))
                or address.get(self.tr("town"))
                or address.get(self.tr("village"))
                or address.get(self.tr("hamlet"), self.tr("Lieu inconnu"))
            )
            return city
...
def create_gps_map(lat, lon, width, height, output_path):
    ...
    if not staticmaps.cairo_is_supported():
        print(self.tr("⚠️ Cairo n'est pas supporté. Impossible de générer la carte."))
        return False
