...
def get_pdf_theme_css(settings_manager) -> str:
    ...
    css_pdf_path = base_path / self.tr("resources") / self.tr("css_pdf") / pdf_theme_filename
    css_preview_path = base_path / self.tr("resources") / self.tr("css_preview") / pdf_theme_filename

    css_path_to_load = css_pdf_path if css_pdf_path.exists() else css_preview_path

    content_css_string = self.tr("")
...
class PdfExportWorker(QRunnable):
    ...
    def _build_html_and_css(
        ...
        all_html_content = self.tr("")

        # 1. Page de garde
        cover_image_path = options.get(self.tr("cover_image"), self.tr(""))
        image_abs_path = self.tr("")
        ...
        pdf_author = options.get(self.tr("author"), self.tr(""))
        author_html = (
            self.tr("<p class="cover-author">Auteur : %1</p>").arg(pdf_author) if pdf_author else self.tr("")
        )

        # V2.9.2 - Ajouter le tag de filtre sur la page de garde s'il existe
        selected_tag = options.get(self.tr("selected_tag"))
        tag_html = (
        ...
        for note_date, html_note in notes_data:
            ...
            all_html_content += self.tr("\n            <div class=\"journal-entry\">\n                <h2 class=\"entry-date\">%1</h2>\n                %2\n            </div>\n            ").arg(date_formatted).arg(html_note)
        ...
        try:
            ...
            print(self.tr("⚠️ Fichier CSS par défaut non trouvé : %1").arg(base_css_path))
            base_weasyprint_css = self.tr("")

        # Gérer le titre dynamique qui ne peut pas être dans le fichier CSS statique
        escaped_pdf_title = options.get(self.tr("title"), self.tr("")).replace(self.tr('"'), self.tr('\\"'))
        ...
        # 4. HTML final
        full_html = self.tr("\n        <!DOCTYPE html>\n        <html lang=\"fr\">\n        <head><meta charset=\"UTF-8\"><title>%1</title></head>\n        <body>%2</body>\n        </html>\n        ").arg(options.get(self.tr('title'), self.tr('Journal'))).arg(all_html_content)
    ...
    def run(self):
        ...
        try:
            ...
            self.signals.error.emit(
                self.tr("La bibliothèque 'WeasyPrint' est requise.\n")
                self.tr("Installez-la avec : pip install WeasyPrint")
            )
...
def export_single_pdf(main_window):
    ...
    try:
        ...
        QMessageBox.critical(
            main_window,
            self.tr("Module manquant"),
            self.tr("WeasyPrint n'est pas installé.\n\n")
            self.tr("Pour utiliser cette fonctionnalité, installez-le avec:\n")
            self.tr("pip install weasyprint"),
        )
        return
    ...
    last_pdf_dir = main_window.settings_manager.get(self.tr("pdf.last_directory"))
    if not last_pdf_dir or not Path(last_pdf_dir).is_dir():
        last_pdf_dir = str(Path.home())

    default_filename = self.tr("%1.pdf").arg(clean_filename)
    ...
    try:
        ...
        if main_window.journal_directory and main_window.current_file:
            ...
            for img in soup.find_all(self.tr("img")):
                src = img.get(self.tr("src"))
                if src and not src.startswith(
                    (self.tr("http://"), self.tr("https://"), self.tr("data:"), self.tr("file://"))
                ):
                    full_path = (main_window.journal_directory / src).resolve()
                    if full_path.exists():
                        img[self.tr("src")] = str(full_path)
            for link in soup.find_all(self.tr("a")):
                href = link.get(self.tr("href"))
                if href and not href.startswith(
                    (self.tr("http://"), self.tr("https://"), self.tr("data:"), self.tr("file://"), self.tr("#"))
                ):
                    full_path = (main_window.journal_directory / href).resolve()
                    if full_path.exists():
                        link[self.tr("href")] = str(full_path)
        ...
        main_window.statusbar.showMessage(self.tr("Exporté en PDF : %1").arg(filename), 3000)
        main_window.settings_manager.set(
            self.tr("pdf.last_directory"), str(Path(filename).parent)
        )
        main_window.settings_manager.save_settings()
    except Exception as e:
        QMessageBox.critical(
            main_window, self.tr("Erreur"), self.tr("Impossible d'exporter en PDF :\n%1").arg(str(e))
        )
...
def export_journal_to_pdf(main_window):
    ...
    if tags_index_path.exists():
        try:
            ...
            print(self.tr("⚠️ Erreur de lecture de l'index des tags : %1").arg(e))
    ...
    last_title = main_window.settings_manager.get(
        self.tr("pdf.last_title"), self.tr("BlueNotebook Journal")
    )
    default_logo_path = (
...
def export_journal_to_pdf_worker(main_window, options, note_files, tags_index_path):
    start_date_q = options[self.tr("start_date")]
    end_date_q = options[self.tr("end_date")]
    selected_tag = options.get(self.tr("selected_tag"))
    ...
    if selected_tag:
        try:
            with open(tags_index_path, "r", encoding="utf-8") as f:
                tags_data = json.load(f)
            files_with_tag = {d[self.tr("filename")] for d in tags_data[selected_tag][self.tr("details")]}
            filtered_notes = [note for note in filtered_notes if note in files_with_tag]
        except (KeyError, FileNotFoundError) as e:
            print(self.tr("⚠️ Erreur lors du filtrage par tag : %1").arg(e))
            filtered_notes = []
    ...
    if not filtered_notes:
        QMessageBox.information(
            main_window,
            self.tr("Aucune note"),
            self.tr("Aucune note trouvée pour les critères sélectionnés."),
        )
        return
    ...
    default_filename = self.tr("Journal-%1-%2.pdf").arg(start_date_q.toString(self.tr('ddMMyyyy'))).arg(end_date_q.toString(self.tr('ddMMyyyy')))
    default_path = os.path.join(last_pdf_dir, default_filename)

    pdf_path, _ = QFileDialog.getSaveFileName(
        main_window,
        self.tr("Exporter le journal en PDF"),
        default_path,
        self.tr("Fichiers PDF (*.pdf)"),
    )
    ...
    for note_file in filtered_notes:
        try:
            ...
            print(self.tr("Erreur de lecture du fichier %1: %2").arg(note_file).arg(e))
            continue
    ...
    try:
        ...
        main_window.settings_manager.set(
            self.tr("pdf.last_directory"), str(Path(pdf_path).parent)
        )
        if options[self.tr("author")]:
            main_window.settings_manager.set(self.tr("pdf.last_author"), options[self.tr("author")])
        ...
        QMessageBox.critical(
            ...
            self.tr("Une erreur est survenue lors de la création du PDF :\n%1").arg(str(e)),
        )
