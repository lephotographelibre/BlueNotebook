...
class UrlToMarkdownDialog(QDialog):
    ...
    def __init__(self, initial_url=self.tr(""), parent=None):
        super().__init__(parent)
        self.setWindowTitle(self.tr("Conversion URL/HTML vers Markdown"))
        ...
        url_layout.addWidget(QLabel(self.tr("Chemin ou URL:")))
        self.url_input = QLineEdit(initial_url)
        url_layout.addWidget(self.url_input)
        browse_button = QPushButton(self.tr("Parcourir..."))
        browse_button.clicked.connect(self.browse_file)
        url_layout.addWidget(browse_button)
        layout.addLayout(url_layout)

        # Options
        self.title_checkbox = QCheckBox(self.tr("Ajouter le titre en #"))
        self.title_checkbox.setChecked(True)
        layout.addWidget(self.title_checkbox)

        self.links_checkbox = QCheckBox(self.tr("Conserver les liens Markdown"))
        self.links_checkbox.setChecked(True)
        layout.addWidget(self.links_checkbox)

        self.clean_checkbox = QCheckBox(self.tr("Utiliser Readability pour nettoyer"))
    ...
    def get_options(self):
        ...
        return {
            self.tr("url_or_path"): self.url_input.text().strip(),
            self.tr("add_title"): self.title_checkbox.isChecked(),
            self.tr("keep_links"): self.links_checkbox.isChecked(),
            self.tr("clean_content"): self.clean_checkbox.isChecked(),
        }
...
class UrlToMarkdownWorker(QRunnable):
    ...
    def run(self):
        ...
        url_or_path = self.options[self.tr("url_or_path")]
        ...
        try:
            ...
            markdown_content = converter.convert(
                url=url_for_base,
                title=self.options[self.tr("add_title")],
                links=self.options[self.tr("keep_links")],
                clean=self.options[self.tr("clean_content")],
...
def run_url_to_markdown_conversion(main_window, initial_url=""):
    ...
    if dialog.exec_() == QDialog.Accepted:
        ...
        # Lancer le worker
        main_window.start_task(self.tr("Conversion en Markdown en cours..."))
