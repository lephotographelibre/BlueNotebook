...
class QuoteFetcher:
    ...
    URL = self.tr("https://citations.ouest-france.fr/")
    ...
    def _get_cache_path():
        ...
        cache_dir = Path(
            user_cache_dir(self.tr("BlueNotebook"), self.tr("BlueNotebook"))
        )
        ...
        if cache_file.exists():
            try:
                with open(cache_file, "r", encoding="utf-8") as f:
                    data = json.load(f)
                if data.get(self.tr("date")) == today_str:
                    return data.get(self.tr("quote")), data.get(self.tr("author"))
    ...
    def get_quote_of_the_day():
        ...
        try:
            ...
            # Trouver la citation
            quote_tag = soup.find(self.tr("blockquote"))
            quote = quote_tag.get_text(strip=True) if quote_tag else None

            # Trouver l'auteur
            author_tag = soup.select_one(self.tr(".quotenav .who a"))
            author = (
                author_tag.get_text(strip=True)
                if author_tag
                else self.tr("Auteur inconnu")
            )
            ...
            if quote:
                ...
                cache_data = {
                    self.tr("date"): str(date.today()),
                    self.tr("quote"): quote,
                    self.tr("author"): author,
                }
