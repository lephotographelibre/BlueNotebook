...
class TagIndexer(QRunnable):
    ...
    def run(self):
        ...
        try:
                        ...
                        for line_number, line in enumerate(f, 1):
                            for match in self.tag_pattern.finditer(line):
                                ...
                                nfkd_form = unicodedata.normalize(
                                    self.tr("NFKD"), tag_body
                                )
                                without_accents = self.tr("").join(
                                    [
                                        c
                                        for c in nfkd_form
                                ...
                                all_tags_info.append(
                                    {
                                        self.tr("tag"): normalized_tag,
                                        self.tr("context"): context,
                                        self.tr("filename"): file_path.name,
                                        self.tr("line"): line_number,
                                    }
                                )
            ...
            all_tags_info.sort(key=lambda x: x[self.tr("tag")])
    ...
    def _write_text_index(self, all_tags_info):
        ...
        index_file_path = self.journal_directory / self.tr("index_tags.txt")
        if index_file_path.exists():
            backup_path = self.journal_directory / self.tr("index_tags.txt.SAVE")
            try:
        ...
        all_tags_data = [
            self.tr("%1++%2++%3")
            .arg(info[self.tr("tag")])
            .arg(info[self.tr("context")])
            .arg(info[self.tr("filename")])
    ...
    def _write_csv_index(self, all_tags_info):
        ...
        index_file_path = self.journal_directory / self.tr("index_tags.csv")
        if index_file_path.exists():
            backup_path = self.journal_directory / self.tr("index_tags.csv.SAVE")
            try:
                index_file_path.rename(backup_path)
            except OSError:
                pass
    ...
    def _write_json_index(self, all_tags_info):
        ...
        json_data = {}
        for info in all_tags_info:
            tag = info[self.tr("tag")]
            if tag not in json_data:
                json_data[tag] = {self.tr("occurrences"): 0, self.tr("details"): []}

            # Extraire et formater la date depuis le nom de fichier
            date_str = self.tr("")
            filename_base = Path(info[self.tr("filename")]).stem  # ex: "20240928"
            if len(filename_base) == 8 and filename_base.isdigit():
                year = filename_base[0:4]
                month = filename_base[4:6]
                day = filename_base[6:8]
                date_str = (
                    self.tr("%1-%2-%3").arg(year).arg(month).arg(day)
                )  # Format ISO 8601

            json_data[tag][self.tr("details")].append(
                {
                    self.tr("context"): info[self.tr("context")],
                    self.tr("filename"): info[self.tr("filename")],
                    self.tr("date"): date_str,
                    self.tr("line"): info[self.tr("line")],
                }
            )
            json_data[tag][self.tr("occurrences")] += 1
...
def start_tag_indexing(journal_directory, pool, on_finished):
    ...
    if journal_directory:
        print(
            self.tr("🚀 Lancement de l'indexation des tags pour : %1").arg(
                journal_directory
            )
        )
