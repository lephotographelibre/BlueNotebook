...
def handle_insert_bookmark(main_window):
    ...
    if selected_text and selected_text.lower().startswith(("http", "ftp")):
        ...
        text, ok = QInputDialog.getText(
            main_window, "Insérer un Bookmark", "Entrez l'URL de la page :"
        )
        if ok and text:
            url = text.strip()
    ...
    def on_bookmark_finished(url, title):
        ...
        if title:
            link_text = f"{prefix}{title} - {url}"
        else:
            link_text = f"{prefix}{url}"
        markdown_link = f"{icon} [{link_text}]({url})"

        cursor = main_window.editor.text_edit.textCursor()
        if cursor.hasSelection():
            cursor.removeSelectedText()
        main_window.editor.insert_text(markdown_link)
        main_window.statusbar.showMessage("Bookmark inséré avec succès.", 3000)
